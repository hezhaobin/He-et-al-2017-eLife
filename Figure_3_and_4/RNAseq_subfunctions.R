...
# define function for annotating linear regression 
lm_eqn = function(m) {
...
# a useful crossbar summary for ggplot2
...
myPlotCounts <- function(gene){
  # remove PHO80wt, hi/noPi samples before plotting
  d <- plotCounts( dds1[,-c(29,30,31,32)], gene = gene, intgroup = c("Pho4","Pho2"), returnData = T )
  d <- droplevels(subset(d, Pho2 != "Cgla2"))
  p <- ggplot( d, aes(x=Pho2, y=count) )
  p <- p + geom_point( size=2 ) + ggtitle(label = all.genes[gene]) + 
    stat_summary( fun.y="mean",geom="point",col="red",size=3 ) + 
    facet_wrap(~Pho4, nrow = 1, scales = "free_x")
  print( p )
}
...
myPlotPHO4 <- function(fit, sig, pho4, unannotated = F, ...){
  ...
  pho4.w.sc2 <- paste(pho4,".w.sc2",sep="")
  ...
  with(dat, 
       plot( x = alone, y = w.sc2, pch = 16, cex = 1.2,
             ...
             xlab = xlab, ylab = ylab, ... ))
  legend(2.5,-1,pch=16,col=rgb(c(1,0,1,0),0,c(0,1,1,0)),bty="n",
